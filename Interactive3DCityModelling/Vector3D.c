...
Vector3D RotateAboutY(const Vector3D* v, float angle) {
	float radians = angle * ((float)M_PI / 180.0f);

	float x = v->x * (float)cos(radians) + v->z * (float)sin(radians);
	float z = v->z * (float)cos(radians) - v->x * (float)sin(radians);

	return NewVector3D(x, 1.0f, z);
}
